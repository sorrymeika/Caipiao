define('views/ssqBuy',['zepto','ui/sl','util','app','views/loading'],function(require,exports,module) {
    var $=require('zepto'),
        sl=require('ui/sl'),
        app=require('app'),
        util=require('util'),
    ...
    module.exports=sl.Activity.extend({
        ...
        buy: function() {
            ...
            sl.prompt('请输入您的投注密码',function(res) {
                ...
                var ssqBetData=localStorage.ssqBetData.split('#'),
                    resultCode=[],
                    codes;

                $.each(ssqBetData,function(i,code) {
                    codes=code.split('|');
        ...
        del: function(e) {
            var that=this,
                ...
                data=localStorage.ssqBetData.split('#');

            data.splice($parent.index(),1);

            localStorage.ssqBetData=data.join('#');
        ...
        onCreate: function() {
            ...
            if(!localStorage.ssqBetData) {
                ...
                $.each(ssqBetData,function(i,item) {
                    betData=item.split('|');
                    ...
                    if(betData[1]=='01') {
                        ...
                        itemData.blue=betData[3].substr(13,4);
                        itemData.red=replaceCode(itemData.red);

                    } else if(betData[1]=='02') {

                        var red=parseInt(betData[3].substr(0,2)),
                            blue=parseInt(betData[3].substr(red*2+2,2));

                        itemData.red=betData[3].substr(2,red*2);
                        itemData.blue=betData[3].substr(red*2+4,blue*2);

                        itemData.num=util.C(red,red-6)*blue;
                        itemData.red=replaceCode(itemData.red);
                        ...
                        var red=parseInt(betData[3].substr(0,2)),
                            ...
                            blue=parseInt(betData[3].substr(red*2+red1*2+4,2));

                        itemData.red=betData[3].substr(2,red*2);
                        itemData.red1=betData[3].substr(red*2+4,red1*2);
                        itemData.blue=betData[3].substr(red*2+red1*2+6,blue*2);

                        itemData.red='('+replaceCode(itemData.red)+')'+replaceCode(itemData.red1);

                        itemData.num=util.C(red1,6-red)*blue;
                    }

                    itemData.blue=replaceCode(itemData.blue);
